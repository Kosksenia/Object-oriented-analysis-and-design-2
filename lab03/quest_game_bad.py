...
class RPGGame:
    ...
    # ============ ВСЕ ПРОВЕРКИ КВЕСТОВ ВЫПОЛНЯЮТСЯ НАПРЯМУЮ В КОДЕ ============
    # Это main() метода attempt_quest в обычной игре, но здесь логика размазана по всему коду
    
    def check_quest_requirements_directly(self, quest: Quest) -> tuple:
        """Прямая проверка условий квеста - БЕЗ использования паттерна.
           Этот метод показывает все недостатки прямого подхода."""
        
        # ПРОВЕРКА УРОВНЯ
        if quest.required_level > 0 and self.player.level < quest.required_level:
            return False, f"❌ Твой уровень слишком низок! Нужно достичь {quest.required_level} уровня. (Твой уровень: {self.player.level})"
        
        # ПРОВЕРКА ПРЕДМЕТА
        if quest.required_item and quest.required_item not in self.player.inventory:
            return False, f"❌ У тебя нет {quest.required_item}! Выполни задание кузнеца."
        
        # ПРОВЕРКА ПРЕДЫДУЩИХ КВЕСТОВ (сложная логика с циклами)
        if quest.required_quest or quest.required_quests:
            ...
            for required_id in required_ids:
                if required_id not in self.player.completed_quests:
                    return False, f"❌ Ты не прошёл квест {required_id}!"
        
        # ПРОВЕРКА РЕПУТАЦИИ
        if quest.required_reputation > 0 and self.player.reputation.get("Гильдия искателей", 0) < quest.required_reputation:
            current = self.player.reputation.get("Гильдия искателей", 0)
            return False, f"❌ Гильдия не доверяет тебе! Нужно {quest.required_reputation} очков репутации. (У тебя: {current})"
        
        # Если все проверки пройдены - выдаём награду
        self.player.add_experience(quest.reward_exp)
        self.player.add_reputation("Гильдия искателей", quest.reward_reputation)
        if quest.reward_item:
            self.player.add_item(quest.reward_item)
        self.player.gold += quest.reward_gold
        self.player.potions += quest.reward_potions
        self.player.completed_quests.append(quest.id)
        
        self.show_floating_text(f"+{quest.reward_exp} опыта!", "#00b894")
        self.show_floating_text(f"+{quest.reward_gold} золота!", "#fdcb6e")
        if quest.reward_item:
            self.show_floating_text(f"+{quest.reward_item}!", "#0984e3")
        
        return True, "✅ Квест выполнен!"
    
    # ============ КОНЕЦ ПРЯМЫХ ПРОВЕРОК ============
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="ВЕРСИЯ БЕЗ ПАТТЕРНА - проверки квестов выполняются напрямую",
                font=('Arial', 12), fg=self.colors['error'], bg=self.colors['bg']).pack()
        tk.Label(title_frame, text="ESC - выход из полноэкранного режима",
    ...
    def update_quest_list(self):
        ...
        for quest_id, display_name in available_quests:
            ...
            if quest.required_level > 0:
                status = "✅" if self.player.level >= quest.required_level else f"❌ ({self.player.level}/{quest.required_level})"
                req_texts.append(f"{status} 🔰 Уровень {quest.required_level}+")
            
            if quest.required_item:
                status = "✅" if quest.required_item in self.player.inventory else "❌"
                req_texts.append(f"{status} 🗡️ {quest.required_item}")
            
            if quest.required_quest or quest.required_quests:
                required_ids = quest.required_quests.copy()
                if quest.required_quest:
                    required_ids.append(quest.required_quest)
                all_completed = all(req_id in self.player.completed_quests for req_id in required_ids)
                status = "✅" if all_completed else "❌"
                req_texts.append(f"{status} 📜 Предыдущие квесты")
            
            if quest.required_reputation > 0:
                current = self.player.reputation.get("Гильдия искателей", 0)
                status = "✅" if current >= quest.required_reputation else f"❌ ({current}/{quest.required_reputation})"
                req_texts.append(f"{status} 🤝 Репутация {quest.required_reputation}+")
            
            req_frame = tk.Frame(quest_frame, bg=self.colors['frame'])
            req_frame.pack(fill=tk.X, padx=5, pady=2)
            for req_text in req_texts:
                color = self.colors['info'] if "✅" in req_text else self.colors['error']
                tk.Label(req_frame, text=req_text, font=('Arial', 8), fg=color, bg=self.colors['frame']).pack(side=tk.LEFT, padx=(0, 10))
            ...
            if closest_enemy.hp <= 0:
                ...
                if closest_enemy.type == "wolf":
                    ...
                    if self.farmer_wolves_killed >= 3 and "quest_farmer" not in self.player.completed_quests:
                        ...
                        success, message = self.check_quest_requirements_directly(quest)
                        if success:
                            self.status_bar.config(text="✅ Квест 'Помощь фермеру' выполнен!")
                    ...
                    if self.forester_bandits_killed >= 2 and "quest_forester" not in self.player.completed_quests:
                        ...
                        success, message = self.check_quest_requirements_directly(quest)
                        if success:
                            self.status_bar.config(text="✅ Квест 'Защита леса' выполнен!")
    ...
    def show_dialog(self, npc):
        ...
        if npc.quest_id == "quest_sword" and self.iron_ores >= 5 and "Меч героя" not in self.player.inventory:
            ...
            self.status_bar.config(text="🗡️ Кузнец выковал тебе Меч героя!")
            self.dialog_label.config(text="Кузнец: 'Вот твой меч!'")
            self.show_floating_text("Получен Меч героя!", "#fdcb6e")
            self.update_stats()
            self.draw_world()
            return
        
        # Квест на репутацию - прямая проверка
        if npc.quest_id == "quest_reputation" and "quest_reputation" not in self.player.completed_quests:
            ...
            if success:
                ...
                self.dialog_label.config(text="Староста: 'Добро пожаловать в Гильдию!'")
...
📜 ВЫПОЛНЕННЫЕ КВЕСТЫ:
    ...
    def start_boss_fight_window(self):
        # Прямая проверка условий для битвы с боссом
        boss_quest = self.quests["quest_boss"]
        success, message = self.check_quest_requirements_directly(boss_quest)
    ...
    def boss_attack(self, battle_window):
        ...
        if self.boss.is_defeated:
            ...
            # Прямая выдача награды за победу над боссом
            boss_quest = self.quests["quest_boss"]
            success, message = self.check_quest_requirements_directly(boss_quest)
