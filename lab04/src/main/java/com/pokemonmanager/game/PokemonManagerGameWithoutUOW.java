...
public class PokemonManagerGameWithoutUOW extends JFrame {
    ...
    public PokemonManagerGameWithoutUOW() {
    ...
    private void initGameGUI() {
        setTitle("POKEMON MANAGER - БЕЗ ПАТТЕРНА (РЕЖИМ РИСКА)");
        ...
        rightPanel.setBackground(new Color(100, 50, 50));
        
        pokemonPanel = new JPanel();
        pokemonPanel.setLayout(new BoxLayout(pokemonPanel, BoxLayout.Y_AXIS));
        pokemonPanel.setBackground(new Color(100, 50, 50));
        ...
        logArea.setBackground(new Color(80, 40, 40));
        logArea.setForeground(Color.RED);
        JScrollPane logScroll = new JScrollPane(logArea);
        logScroll.setPreferredSize(new Dimension(250, 150));
        logScroll.setBorder(BorderFactory.createTitledBorder("ЖУРНАЛ (РЕЖИМ РИСКА)"));
        ...
        log("ВНИМАНИЕ: РЕЖИМ БЕЗ ПАТТЕРНА!");
        log("Каждая операция выполняется отдельно");
        log("При ошибке данные могут быть повреждены!");
    ...
    private JPanel createTopPanel() {
        ...
        panel.setBackground(new Color(120, 60, 60));
    ...
    private JPanel createButtonPanel() {
        ...
        panel.setBackground(new Color(140, 70, 70));
        panel.setBorder(BorderFactory.createEmptyBorder(10, 20, 10, 20));
        
        JButton catchBtn = createButton("ПОЙМАТЬ", new Color(200, 50, 50));
        JButton evolveBtn = createButton("ЭВОЛЮЦИЯ", new Color(200, 100, 50));
        JButton tradeBtn = createButton("ОБМЕН", new Color(150, 100, 150));
        JButton battleBtn = createButton("БИТВА", new Color(200, 50, 50));
        JButton shopBtn = createButton("МАГАЗИН", new Color(200, 150, 50));
        JButton refreshBtn = createButton("ОБНОВИТЬ", new Color(120, 120, 120));
    ...
    private void catchNearbyPokemon() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this,
            "Дикий " + nearest.getName() + " (Lv." + nearest.getLevel() + ") рядом!\n\nПоймать?\n\nВНИМАНИЕ: Режим без транзакций - риск повреждения данных!",
    ...
    private void performCatch(Pokemon p) {
        log("=== ЛОВЛЯ " + p.getName() + " (РЕЖИМ БЕЗ ТРАНЗАКЦИЙ) ===");
        log("ВНИМАНИЕ: Каждая операция выполняется отдельно!");
        
        try {
            log("Шаг 1: Обновление владельца покемона...");
            String updatePokemon = "UPDATE pokemons SET trainer_id = ?, is_wild = 0 WHERE id = ?";
            try (PreparedStatement ps = connection.prepareStatement(updatePokemon)) {
                ps.setInt(1, currentTrainer.getId());
                ps.setInt(2, p.getId());
                ps.executeUpdate();
            }
            ...
            log("Шаг 2: Обновление счётчика покемонов тренера...");
            String updateTrainer = "UPDATE trainers SET pokemon_count = pokemon_count + 1 WHERE id = ?";
            ...
            log("Шаг 3: Добавление достижения...");
            String insertAchievement = "INSERT INTO achievements (trainer_id, achievement_name, is_unlocked) VALUES (?, 'Первая поимка', 1)";
            try (PreparedStatement ps = connection.prepareStatement(insertAchievement)) {
                ps.setInt(1, currentTrainer.getId());
                ps.executeUpdate();
            }
            log("Шаг 3: Выполнен");
            ...
            log("ДАННЫЕ МОГУТ БЫТЬ ПОВРЕЖДЕНЫ - Некоторые шаги выполнены, некоторые нет!");
            JOptionPane.showMessageDialog(this, 
                "ОШИБКА! Данные могут быть повреждены!\n\n" +
                "Покемон может быть частично пойман.\n" +
                "Используйте режим С ПАТТЕРНОМ для безопасных операций!",
    ...
    private void evolvePokemon() {
        ...
        if (choice >= 0) {
            ...
            int confirm = JOptionPane.showConfirmDialog(this,
                oldName + " (Lv." + p.getLevel() + ") -> " + newName + "\n\nStart evolution?\n\nWARNING: Without transactions - risk of data corruption!",
                "Evolution", JOptionPane.YES_NO_OPTION);
        
            if (confirm == JOptionPane.YES_OPTION) {
                showEvolutionAnimation(oldName, newName);
            
                log("=== EVOLVING " + oldName + " (WITHOUT TRANSACTIONS) ===");
            
                try {
                    log("Step 1: Changing name to " + newName + "...");
                    String updateName = "UPDATE pokemons SET name = ? WHERE id = ?";
                    ...
                    log("Step 2: Updating evolution stage and stats...");
                    String updateStats = "UPDATE pokemons SET evolution_stage = ?, level = level + 5, attack = attack + 15, defense = defense + 10, hp = hp + 20, max_hp = max_hp + 20 WHERE id = ?";
                    try (PreparedStatement ps = connection.prepareStatement(updateStats)) {
                    ...
                    log("Step 3: Adding achievement...");
                    String insertAchievement = "INSERT INTO achievements (trainer_id, achievement_name, is_unlocked) VALUES (?, 'Evolution Master', 1)";
                    try (PreparedStatement ps = connection.prepareStatement(insertAchievement)) {
                        ps.setInt(1, currentTrainer.getId());
                        ps.executeUpdate();
                    }
                    ...
                    JOptionPane.showMessageDialog(this,
                        "ERROR! Evolution may be incomplete!\n\n" +
                        "Use WITH PATTERN mode for safe evolution!",
        ...
        if (choice >= 0) {
            ...
            if (targetIdStr != null && !targetIdStr.isEmpty()) {
                try {
                    ...
                    log("=== ОБМЕН " + p.getName() + " (РЕЖИМ БЕЗ ТРАНЗАКЦИЙ) ===");
                    log("ВНИМАНИЕ: 5 отдельных операций - ВЫСОКИЙ РИСК!");
                    
                    try {
                        log("Шаг 1: Создание записи об обмене...");
                        String insertTrade = "INSERT INTO trades (from_trainer_id, to_trainer_id, pokemon_id, status, is_completed) VALUES (?, ?, ?, 'completed', 1)";
                        try (PreparedStatement ps = connection.prepareStatement(insertTrade)) {
                            ps.setInt(1, currentTrainer.getId());
                        ...
                        log("Шаг 2: Передача покемона новому тренеру...");
                        String updatePokemon = "UPDATE pokemons SET trainer_id = ? WHERE id = ?";
                        try (PreparedStatement ps = connection.prepareStatement(updatePokemon)) {
                        ...
                        log("Шаг 3: Уменьшение счётчика покемонов отправителя...");
                        String updateFrom = "UPDATE trainers SET pokemon_count = pokemon_count - 1 WHERE id = ?";
                        try (PreparedStatement ps = connection.prepareStatement(updateFrom)) {
                            ps.setInt(1, currentTrainer.getId());
                            ps.executeUpdate();
                        }
                        log("Шаг 3: Выполнен");
                        
                        log("Шаг 4: Увеличение счётчика покемонов получателя...");
                        String updateTo = "UPDATE trainers SET pokemon_count = pokemon_count + 1 WHERE id = ?";
                        try (PreparedStatement ps = connection.prepareStatement(updateTo)) {
                            ps.setInt(1, targetId);
                            ps.executeUpdate();
                        }
                        log("Шаг 4: Выполнен");
                        
                        log("Шаг 5: Добавление 500 монет отправителю...");
                        String updateMoney = "UPDATE trainers SET money = money + 500 WHERE id = ?";
                        try (PreparedStatement ps = connection.prepareStatement(updateMoney)) {
                            ps.setInt(1, currentTrainer.getId());
                            ps.executeUpdate();
                        }
                        log("Шаг 5: Выполнен");
                        ...
                        log("ОШИБКА: " + e.getMessage());
                        log("ДАННЫЕ МОГУТ БЫТЬ ПОВРЕЖДЕНЫ - Обмен выполнен частично!");
                        JOptionPane.showMessageDialog(this,
                            "ОШИБКА! Обмен выполнен частично!\n\n" +
                            "Покемон может быть потерян или продублирован!\n" +
                            "Используйте режим С ПАТТЕРНОМ для безопасного обмена!",
                            "Критическая ошибка", JOptionPane.ERROR_MESSAGE);
                    }
    ...
    private void startBattle() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            ...
            if (win > 20) {
                ...
                log("=== РЕЗУЛЬТАТ БИТВЫ: ПОБЕДА! (РЕЖИМ БЕЗ ТРАНЗАКЦИЙ) ===");
                log("ВНИМАНИЕ: Каждая операция выполняется отдельно!");
                
                try {
                    log("Шаг 1: Обновление опыта покемона...");
                    String updatePokemon = "UPDATE pokemons SET exp = exp + ?, level = level + 1 WHERE id = ?";
                    try (PreparedStatement ps = connection.prepareStatement(updatePokemon)) {
                        ps.setInt(1, expGain);
                        ps.setInt(2, myP.getId());
                        ps.executeUpdate();
                    }
                    log("Шаг 1: Выполнен");
                    
                    log("Шаг 2: Обновление денег и опыта тренера...");
                    String updateTrainer = "UPDATE trainers SET exp = exp + ?, money = money + ? WHERE id = ?";
                    try (PreparedStatement ps = connection.prepareStatement(updateTrainer)) {
                        ps.setInt(1, expGain / 2);
                        ps.setInt(2, moneyGain);
                        ps.setInt(3, currentTrainer.getId());
                        ps.executeUpdate();
                    }
                    log("Шаг 2: Выполнен");
                    
                    log("Шаг 3: Запись битвы...");
                    String insertBattle = "INSERT INTO battles (trainer1_id, trainer2_id, winner_id, reward, battle_date) VALUES (?, ?, ?, ?, datetime('now'))";
                    try (PreparedStatement ps = connection.prepareStatement(insertBattle)) {
                        ps.setInt(1, currentTrainer.getId());
                        ps.setInt(2, 999);
                        ps.setInt(3, currentTrainer.getId());
                        ps.setInt(4, moneyGain);
                        ps.executeUpdate();
                    }
                    log("Шаг 3: Выполнен");
                    ...
                    log("ОШИБКА: " + e.getMessage());
                    log("ДАННЫЕ МОГУТ БЫТЬ ПОВРЕЖДЕНЫ - Результаты битвы сохранены не полностью!");
                    JOptionPane.showMessageDialog(this,
                        "ОШИБКА! Результаты битвы сохранены не полностью!\n\n" +
                        "Используйте режим С ПАТТЕРНОМ для безопасных битв!",
                        "Предупреждение", JOptionPane.WARNING_MESSAGE);
                }
            } else {
                log("=== РЕЗУЛЬТАТ БИТВЫ: ПОРАЖЕНИЕ ===");
    ...
    private void openShop() {
        ...
        int choice = JOptionPane.showOptionDialog(this,
            "МАГАЗИН - Денег: " + currentTrainer.getMoney() + " монет\n\nВыберите товар:\n\nВНИМАНИЕ: Режим без транзакций - риск потери денег!",
            "Магазин", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE,
            null, items, items[0]);
        
        if (choice >= 0 && currentTrainer.getMoney() >= prices[choice]) {
            log("=== ПОКУПКА " + items[choice] + " (РЕЖИМ БЕЗ ТРАНЗАКЦИЙ) ===");
            log("ВНИМАНИЕ: Каждая операция выполняется отдельно!");
            
            try {
                log("Шаг 1: Списание денег...");
                String updateMoney = "UPDATE trainers SET money = money - ? WHERE id = ?";
                try (PreparedStatement ps = connection.prepareStatement(updateMoney)) {
                    ps.setInt(1, prices[choice]);
                    ps.setInt(2, currentTrainer.getId());
                    ps.executeUpdate();
                }
                log("Шаг 1: Выполнен");
                
                log("Шаг 2: Добавление предмета в инвентарь...");
                String updateInventory = "UPDATE inventory SET quantity = quantity + 1 WHERE trainer_id = ? AND item_name = ?";
                try (PreparedStatement ps = connection.prepareStatement(updateInventory)) {
                    ps.setInt(1, currentTrainer.getId());
                    ps.setString(2, itemNames[choice]);
                    int rows = ps.executeUpdate();
                    if (rows == 0) {
                        String insert = "INSERT INTO inventory (trainer_id, item_name, quantity) VALUES (?, ?, 1)";
                        try (PreparedStatement ps2 = connection.prepareStatement(insert)) {
                            ps2.setInt(1, currentTrainer.getId());
                            ps2.setString(2, itemNames[choice]);
                            ps2.executeUpdate();
                        }
                    }
                }
                ...
                log("ОШИБКА: " + e.getMessage());
                log("ДАННЫЕ МОГУТ БЫТЬ ПОВРЕЖДЕНЫ - Деньги списаны, но предмет не получен!");
                JOptionPane.showMessageDialog(this,
                    "ОШИБКА! Деньги списаны, но предмет не получен!\n\n" +
                    "Используйте режим С ПАТТЕРНОМ для безопасных покупок!",
                    "Критическая ошибка", JOptionPane.ERROR_MESSAGE);
            }
        } else if (choice >= 0) {
    ...
    private void updatePokemonList() {
        ...
        for (Pokemon p : myPokemons) {
            ...
            card.setBackground(new Color(120, 70, 70));
            ...
            infoPanel.setBackground(new Color(120, 70, 70));
